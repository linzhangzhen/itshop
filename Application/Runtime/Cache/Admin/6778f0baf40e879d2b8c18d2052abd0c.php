...
<div  style="height:100%;">
  <ul id="navigation">
    <?php if(is_array($authinfoA)): foreach($authinfoA as $key=>$v): ?><li> <a class="head"><?php echo ($v["auth_name"]); ?></a>
      <ul>
        <?php if(is_array($authinfoB)): foreach($authinfoB as $key=>$vv): ?><!--显示当前父级权限对应的子级权限（条件：当前权限pid=父级权限id）-->
          <?php if(($vv["auth_pid"]) == $v["auth_id"]): ?><li><a href="/index.php/Admin\<?php echo ($vv["auth_c"]); ?>\<?php echo ($vv["auth_a"]); ?>" target="rightFrame"><?php echo ($vv["auth_name"]); ?></a></li><?php endif; endforeach; endif; ?>
      </ul>
